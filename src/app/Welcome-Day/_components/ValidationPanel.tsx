...
interface ValidationResult {
  duplicatePhones: Array<{
    phone: string;
    attendees: Array<{ id: string; fullName: string; email: string }>;
  }>;
...
export default function ValidationPanel({ season }: { season?: string }) {
  ...
  const runValidations = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/Welcome-Day/validation${season ? `?season=${season}` : ''}`);
      if (res.ok) {
        ...
        if (data.summary.totalIssues === 0) {
          showToast('✅ Validation completed - No issues found!', 'success');
        } else {
          showToast(
            `⚠️ Validation completed - ${data.summary.totalIssues} issue(s) found`,
            'warning'
          );
        }
      } else {
      ...
      <div className="p-6 space-y-6">
        <div>
          ...
          <p className="text-sm text-gray-600">
            Check for data quality issues in the Welcome Day system
          </p>
        </div>
            ...
            <div className="space-y-3">
              ...
              {summary.totalIssues === 0 && (
                <div className="bg-green-50 border-2 border-green-300 rounded-lg p-8 text-center">
                  ...
                  <h3 className="text-xl font-bold text-green-800 mb-2">All Validations Passed!</h3>
                  <p className="text-green-700">
                    No duplicate records or email mismatches were found in the system.
                  </p>
                </div>
              )}
                      ...
                      {validationResults.duplicatePhones.map((dup, index) => (
                        ...
                        >
                          ...
                          <ul className="space-y-1 text-sm">
                            {dup.attendees.map((attendee, idx) => (
                          ...
                          <ul className="space-y-1 text-sm">
                            {dup.attendees.map((attendee, idx) => (
                              <li key={idx} className="text-yellow-700 pl-4 py-1">
                                <div className="font-medium">{attendee.fullName}</div>
                                <div className="text-xs text-yellow-600">ID: {attendee.id}</div>
                              </li>
                            ))}
                          </ul>
                        </div>
                            ...
                            {dup.attendees.map((attendee, idx) => (
                              <li key={idx} className="text-purple-700 pl-4 py-1">
                                ...
                                <div className="text-xs text-purple-600">
                                  ID: {attendee.id} | Contact Email: {attendee.email}
                                </div>
                              </li>
                            ))}
                          </ul>
          ...
          <div className="flex gap-3">
            <div className="text-sm text-indigo-800">
              ...
              <ul className="space-y-1 text-indigo-700">
                <li>
                  • <strong>Duplicate Phones:</strong> Multiple attendees with the same phone number
                </li>
                <li>
                  • <strong>Duplicate Emails:</strong> Multiple attendees with the same contact
                  email
                </li>
                <li>
                  • <strong>Duplicate Form Emails:</strong> Multiple attendees with the same form
                  submission email
                </li>
                <li>
