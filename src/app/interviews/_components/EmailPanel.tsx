...
export default function EmailPanel({ onSuccess, season, readOnly }: EmailFormProps) {
  ...
  const handleSendTestEmails = async () => {
    ...
    try {
      const res = await fetch(
        `/api/interviews/email/send-test${season ? `?season=${season}` : ''}`,
        {
          method: 'POST',
        }
      );
  ...
  const handleSendUnsentEmails = async () => {
    ...
    try {
      const res = await fetch(
        `/api/interviews/email/send-unsent${season ? `?season=${season}` : ''}`,
        {
          method: 'POST',
        }
      );
  ...
  const handleSendSingleEmail = async () => {
    ...
    try {
      const res = await fetch(
        `/api/interviews/email/send/${memberId.trim()}${season ? `?season=${season}` : ''}`,
        {
          method: 'POST',
        }
      );
    ...
    <div className="bg-white rounded-lg shadow w-full max-w-full">
      <div className="p-6 space-y-6">
        <div>
          ...
          <p className="text-sm text-gray-600">Send interview invitation emails to applicants</p>
        </div>
        ...
        <div className="bg-gradient-to-br from-indigo-50 to-blue-50 border border-indigo-200 rounded-lg p-4">
          ...
          <p className="text-xs text-indigo-700 mt-2">
            Send interview invitation email to a specific member by their ID
          </p>
        </div>
            ...
            <div className="flex items-center gap-3">
              <div>
                ...
                <div className="text-xs text-orange-200 mt-0.5">
                  Sends interview invitations to members with scheduled interviews
                </div>
              </div>
            </div>
            {loading ? (
          ...
          <div className="flex gap-3">
            <div className="text-sm text-blue-800">
              ...
              <ul className="space-y-1 text-blue-700">
                ...
                <li>• Only members with scheduled interviews will receive emails</li>
                <li>• Emails are only sent once per member (tracked by isEmailSend flag)</li>
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6 animate-in fade-in zoom-in duration-200 border border-gray-200">
            ...
            <p className="text-gray-700 mb-6">
              ...
              {confirmAction === 'unsent' && (
                <>
                  Are you sure you want to send interview invitation emails to{' '}
                  <strong>all members</strong> with scheduled interviews who haven&apos;t received
                  emails yet?
                </>
              )}
              {confirmAction === 'single' && (
                <>
                  Are you sure you want to send an interview invitation email to member with ID:{' '}
